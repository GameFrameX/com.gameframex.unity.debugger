...
**Debugger 除錯元件 (Debugger Component)** - 提供除錯日誌相關的看板。
...
2. 直接在 `manifest.json` 的 `dependencies` 節點下添加以下內容：
   ...
   ```
3. 在 Unity 的 `Package Manager` 中使用 `Git URL` 的方式添加庫，地址為：`https://github.com/gameframex/com.gameframex.unity.debugger.git`
